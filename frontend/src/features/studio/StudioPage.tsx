...
export function StudioPage() {
  ...
  useEffect(() => {
    ...
    const loadWorkspace = async () => {
      try {
        const ws = await getDefaultWorkspaceAPI();
              ...
              <div className="border-t border-neutral-200 bg-neutral-50 p-4 rounded-b-xl space-y-4">
                <div className="flex flex-wrap items-center justify-between text-xs text-neutral-500 font-medium">
                  ...
                  <div className="flex space-x-4">
                    <span>
                      Style: {
                        Number.isFinite(currentResult.style_match_score)
                          ? currentResult.style_match_score
                          : Number.isFinite(currentResult.confidence_score)
                            ? Math.round(currentResult.confidence_score * 100)
                            : 0
                      }%
                    </span>
